...
class Song {
  ...
  /// 判断是否 Apple Music 源（不在本 App 直接取流，播放走歌名+歌手换源）
  bool get isApple => source == 'apple';
  ...
  factory Song.fromJson(Map<String, dynamic> json) {
    ...
    if (isBili) {
      ...
      // Apple catalog id 是字符串（数字或 i.l 前缀的资料库 id），
      // 仅作本地主键用 hashCode；取流不靠它（按歌名+歌手换源）
      final aid = _asString(json['id']) ?? '';
      id = aid.isNotEmpty ? aid.hashCode & 0x7FFFFFFF : 0;
    } else {
      id = _asInt(json['id']);
    }
