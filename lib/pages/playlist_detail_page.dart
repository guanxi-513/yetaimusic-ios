    ...
    try {
      if (widget.source == 'soda') {
        ...
        final r = switch (widget.source) {
          ...
          // Apple 详情返回曲目元数据，播放时按歌名+歌手走换源链
          'apple' => await ApiService.applePlaylistDetail(widget.id!),
      ...
      if (!mounted) return;
      // Apple 登录过期：清本地 token，提示并跳登录页；重新登录成功后返回歌单列表
      if (e is NotLoggedInException && widget.source == 'apple') {
        await context.read<AuthState>().appleExpired();
        if (!mounted) return;
        setState(() {
          _loading = false;
          _error = 'Apple Music 登录已过期，请重新登录';
        });
