...
class _PlaylistsPageState extends State<PlaylistsPage>
  ...
  // ---------- Apple Music 歌单 ----------
  List<Playlist> _applePlaylists = const [];
  bool _appleLoading = false;
  String? _appleError;
  bool? _lastAppleLoggedIn;
  ...
  void _maybeLoadApple() {
    final auth = context.read<AuthState>();
    final appleLoggedIn = auth.appleLoggedIn;
    if (_lastAppleLoggedIn != appleLoggedIn) {
      _lastAppleLoggedIn = appleLoggedIn;
      if (appleLoggedIn) {
        _loadApplePlaylists();
      } else {
        setState(() {
          _applePlaylists = const [];
          _appleError = null;
        });
      }
    }
  ...
  Future<void> _loadApplePlaylists() async {
    ...
    try {
      final list = await ApiService.appleUserPlaylists();
      if (!mounted) return;
      setState(() {
        _applePlaylists = list;
        _appleLoading = false;
      });
    } catch (e) {
      if (!mounted) return;
      if (e is NotLoggedInException) {
        // 登录过期：清本地 token，提示并跳转 Apple Music 登录页
        await context.read<AuthState>().appleExpired();
        if (!mounted) return;
        setState(() => _appleLoading = false);
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(
            content: Text('Apple Music 登录已过期，请重新登录'),
            behavior: SnackBarBehavior.floating,
          ),
        );
        Navigator.of(
          context,
        ).push(MaterialPageRoute(builder: (_) => const AppleMusicLoginPage()));
      } else {
        setState(() {
          _appleLoading = false;
          _appleError = 'Apple Music 歌单加载失败：$e';
        });
      }
    }
  }
  ...
  Widget build(BuildContext context) {
    ...
    final appleLogged = auth.appleLoggedIn || auth.appleChecking;
    ...
    if (appleLogged) {
      topSections.add(_buildAppleSection());
    } else {
      bottomSections.add(_buildAppleSection());
    }
  ...
  Widget _buildAppleSection() {
    final auth = context.read<AuthState>();
    if (auth.appleChecking) {
      return SizedBox.shrink(); // 检查中不占位，避免闪烁
    }
    if (!auth.appleLoggedIn) {
      // 未登录：「登录 Apple Music 同步歌单」卡片，点击跳 WebView 登录页
      return GlassCard(
        padding: EdgeInsets.symmetric(horizontal: 14, vertical: 14),
        borderRadius: 18,
        color: Color(0xFFFA57C1).withOpacity(0.12),
        child: Row(
          children: [
            Icon(Icons.apple, color: fgPrimary.withOpacity(0.75), size: 22),
            SizedBox(width: 10),
            Expanded(
              child: Text(
                '登录 Apple Music 后同步你的歌单',
                style: TextStyle(
                  color: fgPrimary.withOpacity(0.7),
                  fontSize: 12,
                  height: 1.4,
    ...
    return Column(
      ...
      children: [
        Row(
          children: [
            ...
            Container(
              padding: EdgeInsets.symmetric(horizontal: 6, vertical: 2),
              decoration: BoxDecoration(
                color: Color(0xFFFA57C1).withOpacity(0.16),
                borderRadius: BorderRadius.circular(6),
                border: Border.all(color: fgPrimary.withOpacity(0.25)),
              ),
              child: Text(
        ...
        if (_appleError != null)
        ...
        else if (_applePlaylists.isEmpty && !_appleLoading)
          Text(
            '暂无歌单',
            style: TextStyle(color: fgPrimary.withOpacity(0.4), fontSize: 12),
          )
          ...
          SizedBox(
            ...
            child: ListView.separated(
              scrollDirection: Axis.horizontal,
              itemCount: _applePlaylists.length,
              separatorBuilder: (_, __) => const SizedBox(width: 12),
              itemBuilder: (context, i) =>
                  _PlaylistCard(playlist: _applePlaylists[i]),
            ),
          ),
      ],
    );
    ...
    try {
      ...
      if (widget.playlist.source == 'soda') {
        ...
        final r = switch (widget.playlist.source) {
          ...
          'apple' => await ApiService.applePlaylistDetail(
            widget.playlist.detailId,
          ),
            ...
            Container(
              ...
              child: Stack(
                children: [
                  ...
                  // 云端音源来源徽标（左上角小标签，酷狗/QQ/汽水/Apple）
