...
class ApiService {
  ...
  static Future<void> loadCookies() async {
    ...
    _appleToken = prefs.getString(_appleTokenPrefsKey) ?? '';
    _appleProfileJson = prefs.getString(_appleProfilePrefsKey) ?? '';
  }
  ...
  // ---- Apple Music 登录态（media-user-token，独立于其他四音源） ----
  static const String _appleTokenPrefsKey = 'apple_media_token';
  static const String _appleProfilePrefsKey = 'apple_profile';
  static String _appleToken = '';
  static String _appleProfileJson = '';

  /// 当前 Apple Music media-user-token（空 = 未登录）
  static String get appleToken => _appleToken;

  /// 本地缓存的 profile JSON（启动时先显示，/apple/status 返回后更新）
  static String get appleProfileJson => _appleProfileJson;

  /// 保存 media-user-token（WebView 登录验证通过后调用）
  static Future<void> setAppleToken(String token) async {
    _appleToken = token;
    final prefs = await SharedPreferences.getInstance();
    await prefs.setString(_appleTokenPrefsKey, token);
  }

  /// 保存 profile JSON（userId/name/storefront）
  static Future<void> setAppleProfile(String json) async {
    _appleProfileJson = json;
    final prefs = await SharedPreferences.getInstance();
    await prefs.setString(_appleProfilePrefsKey, json);
  }

  /// 清除 Apple Music 本地登录态（退出/过期时调用，不影响其他音源）
  static Future<void> clearAppleToken() async {
    _appleToken = '';
    _appleProfileJson = '';
    final prefs = await SharedPreferences.getInstance();
    await prefs.remove(_appleTokenPrefsKey);
    await prefs.remove(_appleProfilePrefsKey);
  }
  ...
  static Future<Map<String, dynamic>> _getJson(
    ...
    try {
      ...
      } else if (path.startsWith('/apple/')) {
        // Apple Music：登录态是单个 media-user-token，按 Cookie 头回传；
        // 为空时不带（后端回退全局登录态）
        if (_appleToken.isNotEmpty) {
          headers['Cookie'] = 'media-user-token=$_appleToken';
        }
      } else {
      ...
      // 网易云请求才处理 Set-Cookie（酷狗/QQ/汽水/Apple 登录态走响应体或专用头）
      if (!path.startsWith('/kugou/') &&
          !path.startsWith('/qq/') &&
  ...
  // ---------------- Apple Music（第 5 音源，WebView 登录） ----------------
  // 登录态：media-user-token（WebView 读 cookie → 上传后端验证 → 本地持久化），
  // 所有 /apple/* 请求带 Cookie: media-user-token=xxx（见 _getJson 分发）。
  // Apple 歌曲不在本 App 直接取流，一律按 歌名+歌手 换源（见 appleSongUrl）。

  /// 提交 media-user-token 给后端验证（query 传参，非 header）。
  /// 成功返回 profile（userId/name/storefront）；
  /// 401（过期/无订阅）抛 NotLoggedInException，其余错误抛 ApiException。
  /// 注意：本方法只负责验证，token/profile 的本地保存由调用方完成。
  static Future<Map<String, dynamic>> appleLoginVerify(String token) async {
    final data = await _getJson('/apple/login/token', {'token': token});
    if (data['loggedIn'] != true) {
      throw ApiException(data['message']?.toString() ?? 'Apple Music 登录失败');
    }
    final p = data['profile'];
    return p is Map ? Map<String, dynamic>.from(p) : <String, dynamic>{};
  }

  /// Apple Music 登录状态 + profile
  static Future<({bool loggedIn, Map<String, dynamic>? profile})>
  appleStatus() async {
    final data = await _getJson('/apple/status');
    final loggedIn = data['loggedIn'] == true;
    final p = data['profile'];
    return (
      loggedIn: loggedIn,
      profile: loggedIn && p is Map ? Map<String, dynamic>.from(p) : null,
    );
  }

  /// 退出 Apple Music：后端清全局登录态（无需 Cookie），再清本地 token/profile
  static Future<void> appleLogout() async {
    try {
      await _getJson('/apple/logout');
    } catch (_) {
      // 服务端失败也照常清本地
    }
    await clearAppleToken();
  }

  /// Apple Music 我的歌单（需登录）。注入 source='apple'。
  static Future<List<Playlist>> appleUserPlaylists() async {
    final data = await _getJson('/apple/user/playlist');
    if (data['loggedIn'] == false) throw const NotLoggedInException();
    final list = (data['playlists'] as List?) ?? const [];
    return list.map((e) {
      final m = Map<String, dynamic>.from(e as Map);
      m['source'] = 'apple';
  ...
  /// Apple Music 歌单详情（曲目）。歌曲注入 source='apple'。
  /// 返回结构对齐 kugou/qq 详情（四字段 record），供 PlaylistDetailPage 统一消费。
  static Future<
    ({String name, String cover, String description, List<Song> tracks})
  >
  applePlaylistDetail(String id) async {
    final data = await _getJson('/apple/playlist/detail', {'id': id});
    if (data['loggedIn'] == false) throw const NotLoggedInException();
    final tracks = (data['songs'] as List?) ?? const [];
    final songs = tracks.map((e) {
      final m = Map<String, dynamic>.from(e as Map);
      if (m['source'] == null) m['source'] = 'apple';
      return Song.fromJson(m);
    }).toList();
    return (
      name: data['name']?.toString().trim() ?? '',
  ...
  /// Apple 歌曲换源播放：由后端 /apple/song/url 完成完整匹配
  /// （网易云强匹配→正版直取→解锁源→B站兜底，与酷狗/QQ 链路一致）。
  /// 播放列表仍展示 Apple 元数据（封面/歌手），仅音频地址来自换源结果。
  static Future<({String url, int? br, String source, bool unblocked})>
  appleSongUrl({
    required String name,
    required String artist,
    ...
    final data = await _getJson('/apple/song/url', {
      'name': name,
      'artist': artist,
      'duration': duration.toString(),
    });
    final d = data['data'];
    if (d == null || d is! Map) {
      throw ApiException(data['message']?.toString() ?? '换源失败');
    }
    final url = d['url']?.toString() ?? '';
    if (url.isEmpty) {
      throw ApiException('未匹配到可播放音源');
    }
    return (
      url: url,
      br: d['br'] is int ? d['br'] as int : null,
      source: d['source']?.toString() ?? '',
      unblocked: d['unblocked'] == true,
    );
  }
