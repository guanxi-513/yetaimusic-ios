    ...
    public int onStartCommand(Intent intent, int flags, int startId) {
        ...
        WindowManager.LayoutParams params = new WindowManager.LayoutParams(
                ...
                WindowSetup.flag | WindowManager.LayoutParams.FLAG_LAYOUT_NO_LIMITS
                        ...
                        // 锁屏歌词 patch：显示在锁屏之上 + 保持屏幕常亮。
                        // 部分 ROM（ColorOS/MIUI）要求 overlay 带 SHOW_WHEN_LOCKED
                        // 才允许在 keyguard 层渲染。
        ...
        try {
            ...
            // 锁屏歌词诊断：主 App 轮询读取（lock_screen_lyrics_service.dart）
            getSharedPreferences("FlutterSharedPreferences", MODE_PRIVATE)
                    .edit().putString("flutter.lock_overlay_native", "addView 成功").apply();
            ...
            getSharedPreferences("FlutterSharedPreferences", MODE_PRIVATE)
                    .edit().putString("flutter.lock_overlay_native", "addView 失败: BadTokenException").apply();
            isRunning = false;
            detachOverlayView();
            stopSelf(startId);
            return START_NOT_STICKY;
            ...
            getSharedPreferences("FlutterSharedPreferences", MODE_PRIVATE)
                    .edit().putString("flutter.lock_overlay_native", "addView 失败: " + e).apply();
            isRunning = false;
            detachOverlayView();
            stopSelf(startId);
            return START_NOT_STICKY;
